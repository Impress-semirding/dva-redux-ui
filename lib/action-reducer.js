...
var defaultState = exports.defaultState = new _immutable.Map({
  __reducers_ui: new _immutable.Map({
  ...
  switch (action.type) {
    ...
    case MOUNT_UI_STATE:
      ...
      state = state.withMutations(function (s) {
        ...
        // inside the __reducers_ui map.
        if (customReducer) {
          var path = key.join('.');
          s.setIn(['__reducers_ui', path], {
            path: key,
            func: customReducer
          });
        }
    ...
    case UNMOUNT_UI_STATE:
      ...
      state = state.withMutations(function (s) {
        ...
        s.deleteIn(['__reducers_ui', key.join('.')]);
      });
      break;
  }

  var customReducers = state.get('__reducers_ui');
